...
function Rows.CreateCalculatorWidgets(resultRow, index)
    ...
    Calculator:StyleCalculatorButton(calcActionBar, 22)
    calcActionBar:SetScript("OnClick", function()
        Search:SetSelectedIndex(index)
        Search:SetToggleFocused(false)
        Results:UpdateSelectionHighlight(true)
        local data = resultRow.data
        Calculator:OpenCalculator(data and data.calculatorExpression or nil)
    end)
    calcActionBar:Hide()
    ...
    Calculator:SetCalculatorRoundedFill(calcActionKeyCap, 0.13, 0.13, 0.15, 1, 0.30, 0.30, 0.32, 0.95)
