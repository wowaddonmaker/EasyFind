...
### Added
- **EasyFind links**: right-click a result that has no chat link of its own (a setting, a menu, a map spot, a macro, an outfit) and choose EasyFind link to send it to a chat channel, a whisper, or your clipboard. Anyone running EasyFind sees a clickable link that opens that result on their side, the same as clicking it in their own search; everyone else sees plain text. Outfits and macros open where they live (transmog, macro window). Results that already link in chat (items, spells, mounts, achievements) keep their normal link
