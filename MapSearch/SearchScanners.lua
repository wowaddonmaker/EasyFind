...
local POIRules = ns.MapSearchPOIRules
    ...
    if areaPOIs then
        for _, poiID in ipairs(areaPOIs) do
            ...
            if poiInfo and poiInfo.name then
                ...
                local category = POIRules.ResolveAreaPOICategory(poiName, desc)
...
function MapSearch:GetPinInfo(pin)
    ...
    if pin.areaPoiInfo then
        ...
        category = POIRules.ResolvePinAreaPOICategory(poiName, poiDesc)
        if category then
            pinType = category
